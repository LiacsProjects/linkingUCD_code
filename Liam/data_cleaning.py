...
def clean_professor_data():
    profs_df = pd.read_excel(r'excelfiles_L/Hoogleraren all - preprocessed.xlsx', engine='openpyxl').replace(
        {np.nan: None})
    # Split dates
    profs_df[['birth_year', 'birth_month', 'birth_day']] = profs_df['geboortedatum'].str.split('-', expand=True)
    # profs_df[['death_day', 'death_month', 'death_year']] = profs_df['Sterfdatum'].str.split('-', expand=True)
    profs_df[['death_year', 'death_month', 'death_day']] = profs_df['Sterfdatum'].str.split('-', expand=True)
    profs_df[['promotion_year', 'promotion_month', 'promotion_day']] = profs_df['Datum'].str.split('-', expand=True)
    profs_df[['aan1y', 'aan1m', 'aan1d']] = profs_df['Datum aanstelling I'].str.split('-', expand=True)
    profs_df[['amb1y', 'amb1m', 'amb1d']] = profs_df['Ambtsaanvaarding I'].str.split('-', expand=True)
    profs_df[['or1y', 'or1m', 'or1d']] = profs_df['Datum oratie I'].str.split('-', expand=True)
    profs_df[['ed1y', 'ed1m', 'ed1d']] = profs_df['Einde dienstverband I'].str.split('-', expand=True)
    profs_df[['aan2y', 'aan2m', 'aan2d']] = profs_df['Datum aanstelling II'].str.split('-', expand=True)
    profs_df[['amb2y', 'amb2m', 'amb2d']] = profs_df['Ambtsaanvaarding II'].str.split('-', expand=True)
    profs_df[['or2y', 'or2m', 'or2d']] = profs_df['Datum oratie/openbare les II'].str.split('-', expand=True)
    profs_df[['ed2y', 'ed2m', 'ed2d']] = profs_df['Einde dienstverband II'].str.split('-', expand=True)
    profs_df[['aan3y', 'aan3m', 'aan3d']] = profs_df['Datum aanstelling III'].str.split('-', expand=True)
    profs_df[['amb3y', 'amb3m', 'amb3d']] = profs_df['Ambtsaanvaarding III'].str.split('-', expand=True)
    profs_df[['or3y', 'or3m', 'or3d']] = profs_df['Datum oratie/openbare les III'].str.split('-', expand=True)
    profs_df[['ed3y', 'ed3m', 'ed3d']] = profs_df['Einde dienstverband III'].str.split('-', expand=True)
    profs_df[['aan4y', 'aan4m', 'aan4d']] = profs_df['Datum aanstelling IV'].str.split('-', expand=True)
    profs_df[['amb4y', 'amb4m', 'amb4d']] = profs_df['Ambtsaanvaarding IV'].str.split('-', expand=True)
    profs_df[['or4y', 'or4m', 'or4d']] = profs_df['Datum oratie/openbare les IV'].str.split('-', expand=True)
    profs_df[['ed4y', 'ed4m', 'ed4d']] = profs_df['Einde dienstverband IV'].str.split('-', expand=True)
    ...
    all_jobs['end_century'] = all_jobs['end_century'].replace(['na', 'No', 'la', 'to'], '0')
    # html = all_jobs['end_century'].to_frame().to_html()
    # print(html)
    ...
    all_jobs.to_excel('excelfiles_L/professors_all_jobs.xlsx', index=False)
    profs_df.to_excel('excelfiles_L/professors_data.xlsx', index=False)

    # Gender
    gender_df = create_value_counts_professor(profsbirth_df, 'gender', 'Geslacht', 'birth_year', 'birth_century')
    gender_df.to_excel('excelfiles_L/professors_gender.xlsx', index=False)

    # Title
    title_df = create_value_counts_professor(profsbirth_df, 'title', 'Titulatuur 1', 'birth_year', 'birth_century')
    title_df.to_excel('excelfiles_L/professors_title.xlsx', index=False)

    # Birth
    birth_df = create_value_counts_professor(profsbirth_df, 'birth', 'birth_year', 'birth_year', 'birth_century')
    birth_df.to_excel('excelfiles_L/professors_birth.xlsx', index=False)

    # Birthplace
    birthplace_df = create_value_counts_professor(profsbirth_df, 'birth place', 'Geboorteplaats', 'birth_year',
                                                  'birth_century')
    birthplace_df.to_excel('excelfiles_L/professors_birth_place.xlsx', index=False)
    ...
    birthcountry_df.to_excel('excelfiles_L/professors_birth_country.xlsx', index=False)

    # Death
    death_df = create_value_counts_professor(profsdeath_df, 'death', 'death_year', 'death_year', 'death_century')
    death_df.to_excel('excelfiles_L/professors_death.xlsx', index=False)

    # Deathplace
    deathplace_df = create_value_counts_professor(profsdeath_df, 'death place', 'Sterfplaats', 'death_year',
                                                  'death_century')
    deathplace_df.to_excel('excelfiles_L/professors_death_place.xlsx', index=False)
    ...
    deathcountry_df.to_excel('excelfiles_L/professors_death_country.xlsx', index=False)

    # promotion
    promotion_df = create_value_counts_professor(profspromotion_df, 'promotion', 'Datum', 'promotion_year',
                                                 'promotion_century')
    promotion_df.to_excel('excelfiles_L/professors_promotion.xlsx', index=False)
    ...
    promotiontype_df.to_excel('excelfiles_L/professors_promotion_type.xlsx', index=False)

    # Promotionplace
    promotionplace_df = create_value_counts_professor(profspromotion_df, 'promotion place', 'Instelling',
                                                      'promotion_year', 'promotion_century')
    promotionplace_df.to_excel('excelfiles_L/professors_promotion_place.xlsx', index=False)
    ...
    individual_profs_df['Rating'] = individual_profs_df['Rating'].apply(lambda x: '⭐⭐⭐' if x >= 3 else ('⭐⭐' if x >= 2
        ...
        '⭐' if x >= 1 else '')))
    individual_profs_df.to_excel('excelfiles_L/professors_individual.xlsx', index=False)
