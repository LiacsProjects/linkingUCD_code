                    ...
                    for nam_row in nam_rows:
                        ...
                        if len(nam_cells) > 1:
                            ...
                            if nam_cells[0].text.strip() == 'Geboortedatum':
                                bir_cell = nam_cells[1].text.strip()
                                if bir_cell.find(' te ') != -1:
                                    bir = bir_cell.split(' te ')
                                    bir_d = bir[0]
                                    b_day.append(bir_d)
                                    bir_p = bir[1]
                                    b_place.append(bir_p)
                                else:
                                    b_day.append(bir_cell)
                            ...
                            if nam_cells[0].text.strip() == 'Overlijdensdatum':
                                ovr_cell = nam_cells[1].text.strip()
                                if ovr_cell.find(' te ') != -1:
                                    ovr = ovr_cell.split(' te ')
                                    ovr_d = ovr[0]
                                    o_day.append(ovr_d)
                                    ovr_p = ovr[1]
                                    o_place.append(ovr_p)
                                else:
                                    o_day.append(ovr_cell)
...
recmag = pd.DataFrame(recmag_list, columns=('Period', 'Name', 'Picture', 'Picture_saved', 'Term/Details', 'Country',
                                            ...
                                            'Familiar works'))
recmag.to_csv('recmag_v3.csv', index=False, encoding='utf-8-sig')