...
def pivoting(df, index, column, value, fct):
...
datafile = "outputVT1849 extended.csv"
beroepenfile = "Beroepshierarchie.csv"

footer = "Universiteit Leiden, Digital Humanities"
message = "Zicht op de volkstellingcijfers van 1849 op basis van het register van Erfgoed Leiden en Omstreken (archiefnr. 0516-1118)"
                                ...
                                html.Div([
                                          dash_table.DataTable(
                                                               ...
                                                               style_cell={'textAlign': 'center',
                                                                           ...
                                                                           'minWidth': '50px', 'width': '60px', 'maxWidth': '200px',
                                                                           },
...
# dropdown for themes
@app.callback(
    Output('selectthema', 'value'),
    Input('selectthema', 'options'))
def select_thema(options): 
    return options[0]['value']

# dropdown for categories
@app.callback(
    Output('selectcategory', 'options'),
    Input('selectthema', 'value'))
...
@app.callback(
    ...
    Input('selectcategory', 'options'))
def select_category(options): 
    return options[0]['value']

# dropdown for measurements
@app.callback(
    Output('selectmeasure', 'options'),
...
@app.callback(
    Output('selectmeasure', 'value'),
    Input('selectmeasure', 'options'))
...
@app.callback(
    Output('plots', 'figure'),
    Output('map','figure'),
    Output('table_container','columns'),
...
########################################################################### DATA    
    ...
    df_filter['AANTAL'] = [1 for i in range(len(df_filter))]
    df_filter['PERSONEEL'] = df_filter.DIENSTBODES + df_filter.DIENSTPERS
    ...
    if category == "Type gezin":                                cat_data = "SOORT_HUISHOUDEN"
    elif category == 'Religie':                                 cat_data = "RELIGIE"
    elif category == 'Sociale klasse':                          cat_data = "SOC_KLASSE2"    
    elif thema == "Percelen" and category != "Wijken":          cat_data = "SOORT_PERCEEL"  
    elif thema == "Universiteit" and category == 'Beroep':      cat_data = "BEROEP"
    elif category == "Beroep" and not thema == "Universiteit" : cat_data = "BEROEPSGROEP"
    else: cat_data = "WIJKNR"
...
######################################################################################## GRAPH
    ...
    if category == 'Beroep':
      ...
      if (category == "Beroep") and (not thema == "Universiteit"):
         ...
         df_sun = df_aggregate.reset_index(level=['BEROEPSGROEP','BEROEP']) # put grouplevels back from index to frame
...
######################################################################### TABLE
    if category == 'Wijken': cat_data = "SOORT_PERCEEL"
...
# calculate pivot table     
    ...
    type = ['text','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric'] 
    add = ['', "Wijk ", "Wijk ", "Wijk ", "Wijk ", "Wijk ", "Wijk ", "Wijk ", "Wijk ",'']
    columns =[{"name": add[i]+str(table.columns[i]), "id": str(table.columns[i]), 'type': type[i]} for i in range(len(table.columns))] 
